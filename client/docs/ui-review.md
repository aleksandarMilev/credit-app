...
What's missing entirely: anything that says _who_ lends. There is no company name, no ЕИК, no BNB status, no address, and nothing about Bulgaria/EUR beyond the currency symbol.
...
- **Gaps:**
  ...
  - After removing a file, re-selecting the _same_ file won't fire `onChange`, because the input value isn't reset (code-only, `ApplyPage.tsx:283-285`).
- **Fix:**
