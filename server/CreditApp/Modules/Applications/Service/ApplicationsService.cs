...
public class ApplicationsService(
    ...
    IOptions<ApplicationRetentionSettings> retentionSettings,
    ILogger<ApplicationsService> logger) : IApplicationsService
{
    ...
    public async Task<Result> SoftDelete(
        Guid id,
        CancellationToken cancellationToken = default)
    {
        var application = await data
        ...
        var affectedRows = await data
            ...
            .ExecuteUpdateAsync(
                setters => setters
                    .SetProperty(static a => a.IsDeleted, true)
                    .SetProperty(a => a.DeletedOn, deletedOn)
    ...
    public async Task<int> SoftDeleteExpired(
        CancellationToken cancellationToken = default)
    {
        var cutoff = DateTime
            .UtcNow
            .AddDays(-retentionSettings.Value.SoftDeleteAfterDays);

        var deletedOn = DateTime.UtcNow;
        const string SystemActor = "system-retention-worker";

        return await data
            .Applications
            .IgnoreQueryFilters()
            .Where(a => !a.IsDeleted && a.CreatedOn <= cutoff)
            .ExecuteUpdateAsync(
                setters => setters
                    .SetProperty(static a => a.IsDeleted, true)
                    .SetProperty(a => a.DeletedOn, deletedOn)
                    .SetProperty(a => a.DeletedBy, SystemActor),
                cancellationToken);
    }

    public async Task<int> HardDeleteExpired(
        CancellationToken cancellationToken = default)
    {
        var cutoff = DateTime
            .UtcNow
            .AddDays(-retentionSettings.Value.HardDeleteGracePeriodDays);

        var expiredApplications = await data
            .Applications
            .IgnoreQueryFilters()
            .Where(a => a.IsDeleted && a.DeletedOn <= cutoff)
            .ToListAsync(cancellationToken);
